...
namespace Fintrak.Business.Core.Managers
{
    ...
    public class ExtractionProcessManager : ManagerBase, IExtractionProcessService
    {
        ...
        const string GROUP_ACCESS = "Access Maintenance";
        ...
        public SolutionRunDateData[] GetSolutionRunDateByLogin(string loginID)
        {
            return ExecuteFaultHandledOperation(() =>
            {
                var groupNames = new List<string>() { GROUP_ADMINISTRATOR, GROUP_SUPER_BUSINESS, GROUP_BUSINESS, GROUP_USER, GROUP_ACCESS };
                AllowAccessToOperation(SOLUTION_NAME, groupNames);
        ...
        public SolutionRunDate[] GetRunDate()
        {
            return ExecuteFaultHandledOperation(() =>
            {
                var groupNames = new List<string>() { GROUP_ADMINISTRATOR, GROUP_SUPER_BUSINESS, GROUP_BUSINESS, GROUP_ACCESS };
                AllowAccessToOperation(SOLUTION_NAME, groupNames);
        ...
        public ClosedPeriodData[] GetClosedPeriodByLogin(string loginID)
        {
            return ExecuteFaultHandledOperation(() =>
            {
                ...
                IEnumerable<ClosedPeriodInfo> closedPeriodInfos = closedPeriodRepository.GetClosedPeriods().Where(c => solutionIds.Contains(c.ClosedPeriod.SolutionId) && solutionId2s.Contains(c.ClosedPeriod.SolutionId)).ToArray();
        ...
        public UploadData[] GetUploadBySolution_2(int solutionId, string loginID)
        {
            return ExecuteFaultHandledOperation(() =>
            {
                var groupNames = new List<string>() { GROUP_ADMINISTRATOR, GROUP_SUPER_BUSINESS, GROUP_BUSINESS };
                ...
                var roleIds = userRoleRepository.GetUserRoleInfo().Where(c => c.UserSetup.LoginID == loginID).Select(c => c.Role.RoleId).Distinct();
                var uploadIds = uploadRoleRepository.GetUploadRoles().Where(c => roleIds.Contains(c.UploadRole.RoleId)).Select(c => c.Upload.UploadId).Distinct();
                ...
                List<UploadData> uploads = new List<UploadData>();
                //IEnumerable<UploadInfo> uploadInfos = uploadRepository.GetUploads().Where(c => solutionIds.Contains(c.Upload.SolutionId) && c.Upload.Active == true).OrderBy(c => c.Upload.Position).ToArray();
                IEnumerable<UploadInfo> uploadInfos = uploadRepository.GetUploads().Where(c => uploadIds.Contains(c.Upload.UploadId) && solutionIds.Contains(c.Upload.SolutionId)).OrderBy(c => c.Upload.Position).ToArray();
