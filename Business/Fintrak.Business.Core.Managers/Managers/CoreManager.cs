    ...
    public class CoreManager : ManagerBase, ICoreService
    {
        ...
        public Product[] GetProductByCode(string code)
        {
            return ExecuteFaultHandledOperation(() =>
            {
                ...
                IEnumerable<Product> products = productRepository.Get().Where(c => c.Code == code);
                return products.ToArray();
            });
        }
        ...
        #region IncomeCRRSector operations

        [OperationBehavior(TransactionScopeRequired = true)]
        public IncomeCRRSector UpdateIncomeCRRSector(IncomeCRRSector ics)
        {
            return ExecuteFaultHandledOperation(() =>
            {
                var groupNames = new List<string>() { GROUP_ADMINISTRATOR, GROUP_SUPER_BUSINESS, GROUP_BUSINESS };
                ...
                IIncomeCRRSectorRepository icsRepository = _DataRepositoryFactory.GetDataRepository<IIncomeCRRSectorRepository>();

                IncomeCRRSector updatedEntity = null;

                if (ics.ID == 0)
                    updatedEntity = icsRepository.Add(ics);
                else
                    updatedEntity = icsRepository.Update(ics);
        ...
        [OperationBehavior(TransactionScopeRequired = true)]
        public void DeleteIncomeCRRSector(int Id)
        {
            ExecuteFaultHandledOperation(() =>
            {
                var groupNames = new List<string>() { GROUP_ADMINISTRATOR, GROUP_SUPER_BUSINESS, GROUP_BUSINESS };
        ...
        public IncomeCRRSector GetIncomeCRRSector(int Id)
        {
            return ExecuteFaultHandledOperation(() =>
            {
                var groupNames = new List<string>() { GROUP_ADMINISTRATOR, GROUP_SUPER_BUSINESS, GROUP_BUSINESS, GROUP_USER };
                ...
                IncomeCRRSector icsEntity = icsRepository.Get(Id);
                if (icsEntity == null)
                {
                    NotFoundException ex = new NotFoundException(string.Format("IncomeCRRSector with ID of {0} is not in database", Id));
                    throw new FaultException<NotFoundException>(ex, ex.Message);
                }
        ...
        public IncomeCRRSector[] GetAllIncomeCRRSector()
        {
            return ExecuteFaultHandledOperation(() =>
            {
                var groupNames = new List<string>() { GROUP_ADMINISTRATOR, GROUP_BUSINESS, GROUP_USER };
                ...
                IEnumerable<IncomeCRRSector> ics = icsRepository.Get().Where(c => c.Active);
        ...
        #region IncomeMonths operations

        [OperationBehavior(TransactionScopeRequired = true)]
        public IncomeMonths UpdateIncomeMonths(IncomeMonths im)
        {
            return ExecuteFaultHandledOperation(() =>
            {
                var groupNames = new List<string>() { GROUP_ADMINISTRATOR, GROUP_SUPER_BUSINESS, GROUP_BUSINESS };
                ...
                IIncomeMonthsRepository imRepository = _DataRepositoryFactory.GetDataRepository<IIncomeMonthsRepository>();

                IncomeMonths updatedEntity = null;

                if (im.ID == 0)
                    updatedEntity = imRepository.Add(im);
                else
                    updatedEntity = imRepository.Update(im);
        ...
        [OperationBehavior(TransactionScopeRequired = true)]
        public void DeleteIncomeMonths(int Id)
        {
            ExecuteFaultHandledOperation(() =>
            {
                var groupNames = new List<string>() { GROUP_ADMINISTRATOR, GROUP_SUPER_BUSINESS, GROUP_BUSINESS };
                ...
                imRepository.Remove(Id);
            });
        }

        public IncomeMonths GetIncomeMonths(int Id)
        {
            return ExecuteFaultHandledOperation(() =>
            {
                var groupNames = new List<string>() { GROUP_ADMINISTRATOR, GROUP_SUPER_BUSINESS, GROUP_BUSINESS, GROUP_USER };
                ...
                IncomeMonths imEntity = imRepository.Get(Id);
                if (imEntity == null)
                {
                    NotFoundException ex = new NotFoundException(string.Format("IncomeMonths with ID of {0} is not in database", Id));
                    throw new FaultException<NotFoundException>(ex, ex.Message);
                }
        ...
        public IncomeMonths[] GetAllIncomeMonths()
        {
            return ExecuteFaultHandledOperation(() =>
            {
                var groupNames = new List<string>() { GROUP_ADMINISTRATOR, GROUP_BUSINESS, GROUP_USER };
                ...
                IEnumerable<IncomeMonths> im = imRepository.Get().Where(c => c.Active);
