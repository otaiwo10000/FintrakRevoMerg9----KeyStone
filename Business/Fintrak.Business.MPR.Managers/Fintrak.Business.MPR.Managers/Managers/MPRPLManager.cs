    ...
    public class MPRPLManager : ManagerBase, IMPRPLService
    {
                ...
                foreach (var plCaptionInfo in plCaptionInfos)
                {
                    plCaption.Add(
                        new PLCaptionData
                        {
                            ...
                            // ParentName = plCaptionInfo.Parent != null ? plCaptionInfo.Parent.Name : string.Empty,
                            //CompanyCode = "",
                            CompanyCode = plCaptionInfo.PLCaption.CompanyCode,
                            //totalLineInfo.Parent != null ? totalLineInfo.Parent.Name : string.Empty,

                            Summary1 = plCaptionInfo.PLCaption.Summary1,
                            Summary2 = plCaptionInfo.PLCaption.Summary2,
                            Summary3 = plCaptionInfo.PLCaption.Summary3,
                            Summary4 = plCaptionInfo.PLCaption.Summary4,
                            Summary5 = plCaptionInfo.PLCaption.Summary5,
        ...
        public MPRCommFee[] GetMPRCommFees()
        {
            return ExecuteFaultHandledOperation(() =>
            {
                ...
                IEnumerable<MPRCommFee> MPRCommFees = _MPRCommFeeRepository.Get();
        ...
        #region CommFee search manual operations

        [OperationBehavior(TransactionScopeRequired = true)]
        public IncomeAdjustmentCommFeeSearchManual UpdateCommFeeManual(IncomeAdjustmentCommFeeSearchManual cf)
        {
            return ExecuteFaultHandledOperation(() =>
            {
                var groupNames = new List<string>() { GROUP_ADMINISTRATOR, GROUP_USER };
                ...
                IIncomeAdjustmentCommFeeSearchManualRepository cfRepository = _DataRepositoryFactory.GetDataRepository<IIncomeAdjustmentCommFeeSearchManualRepository>();

                IncomeAdjustmentCommFeeSearchManual updatedEntity = null;

                if (cf.ID == 0)
                    updatedEntity = cfRepository.Add(cf);
                else
                    updatedEntity = cfRepository.Update(cf);
        ...
        [OperationBehavior(TransactionScopeRequired = true)]
        public void DeleteCommFeeManual(int Id)
        {
            ExecuteFaultHandledOperation(() =>
            {
                var groupNames = new List<string>() { GROUP_ADMINISTRATOR, GROUP_USER };
                ...
                cfRepository.Remove(Id);
            });
        }

        public IncomeAdjustmentCommFeeSearchManual GetCommFeeManual(int Id)
        {
            return ExecuteFaultHandledOperation(() =>
            {
                var groupNames = new List<string>() { GROUP_ADMINISTRATOR, GROUP_USER };
                ...
                IncomeAdjustmentCommFeeSearchManual cfEntity = cfRepository.Get(Id);
                if (cfEntity == null)
                {
                    NotFoundException ex = new NotFoundException(string.Format("CommFee with ID of {0} is not in database", Id));
                    throw new FaultException<NotFoundException>(ex, ex.Message);
                }
        ...
        public IncomeAdjustmentCommFeeSearchManual[] GetCommFeeManuals()
        {
            return ExecuteFaultHandledOperation(() =>
            {
                var groupNames = new List<string>() { GROUP_ADMINISTRATOR, GROUP_USER };
                ...
                var _cfRepository = new IncomeAdjustmentCommFeeSearchManualRepository();

                IEnumerable<IncomeAdjustmentCommFeeSearchManual> cf = _cfRepository.Get();
        ...
        public IncomeAdjustmentCommFeeSearchManual[] GetCommFeesByYearPeriod(int year, int period, string search)
        {
            return ExecuteFaultHandledOperation(() =>
            {
                var groupNames = new List<string>() { GROUP_ADMINISTRATOR, GROUP_USER };
                ...
                IEnumerable<IncomeAdjustmentCommFeeSearchManual> cf = _cfRepository.GetCommFeesByYearPeriod(year, period, search);
                return cf.ToArray();
            });
        }
