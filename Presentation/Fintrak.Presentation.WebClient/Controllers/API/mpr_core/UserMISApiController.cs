...
namespace Fintrak.Presentation.WebClient.API
{
    ...
    public class UserMISApiController : ApiControllerBase
    {
        ...
        [HttpGet]
        [Route("getusermisdetail/{userMISId}/{LevelId}")]
        [Authorize]
        public HttpResponseMessage GetUserMISDetailInfo(HttpRequestMessage request, int userMISId, int LevelId)
        {
            return GetHttpResponse(request, () =>
            {
                HttpResponseMessage response = null;
                ...
                {
                    var classifications = _MPRCoreService.GetAllTeamClassifications().Where(c => c.ClassificationTypeCode == classificationType.Code && c.Level == LevelId).ToArray();
