...
(function () {
    ...
    angular
        ...
        ['$scope', '$state', '$filter', 'viewModelHelper', 'validator', 'extractionService', '$interval',
                        RunExtractionListController]);

    function RunExtractionListController($scope, $state, $filter, viewModelHelper, validator, extractionService, $interval) {
        var vm = this;
        ...
        vm.cancelExtractions = function () {
            ...
            extractionProgressIndicatorForCancel();
        ...
        vm.exProgLabel = false;
        vm.spinner = false;
        vm.extractionStatus = ""
        vm.extractionHasStarted = false;
        vm.stopChecking = true;
        vm.extractionProgressObj = { PackageName: '', TotalCount: '', CompletedPkgCount: '', EstimatedTimeString: ''};
       
        var extractionProgressFunc = function () {

            vm.stopChecking = false;

            vm.viewModelHelper.apiGet('api/runextraction/extractionprogressinfo', null,
                function (result) {
                    vm.extractionProgressObj = result.data;

                    //vm.exProgLabel = true;                   

                    if (vm.extractionProgressObj.CompletedPkgCount < vm.extractionProgressObj.TotalCount) {
                        if (vm.extractionProgressObj.TotalCount > 0) {
                            vm.exProgLabel = true;
                            vm.spinner = true;

                            vm.extractionStatus = vm.extractionProgressObj.PackageName + ' Extraction is ongoing: ' +
                                vm.extractionProgressObj.CompletedPkgCount + ' of ' + vm.extractionProgressObj.TotalCount + ' completed. ' +
                                ' Estimated Time: ' + vm.extractionProgressObj.EstimatedTimeString;

                            vm.extractionHasStarted = true;
                        }
                    }
                    else {
                        vm.spinner = false;

                        if (vm.extractionHasStarted === true && vm.extractionProgressObj.TotalCount === vm.extractionProgressObj.CompletedPkgCount) {

                            vm.extractionStatus = "Extraction ends. Check log for detail.";
                            vm.stopChecking = true;
                        }
                    }
        ...
        var extractionProgressIndicatorForCancel = function () {
            vm.exProgLabel = false;
            vm.spinner = false;
            vm.extractionStatus = "";
        };
        ...
        //$interval(function () {
        //    //if (vm.extractionProgressObj.CompletedPkgCount < vm.extractionProgressObj.TotalCount && vm.exProgLabel === true)
        //    //if (vm.exProgLabel === true)
        //    if (vm.stopChecking === false)
        //    { extractionProgressFunc();}           
