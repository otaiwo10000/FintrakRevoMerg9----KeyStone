...
(function () {
    ...
    angular
        ...
        ['$scope', '$state', 'viewModelHelper', 'validator', 'processService', '$interval',
                        RunProcessListController]);

    function RunProcessListController($scope, $state, viewModelHelper, validator, processService, $interval) {
        var vm = this;
        ...
        vm.cancelProcesses = function () {
            ...
            processProgressIndicatorForCancel();
        ...
        vm.prProgLabel = false;
        vm.spinner = false;
        vm.processStatus = "";
        vm.processHasStarted = false;
        vm.stopChecking = true;
        vm.processProgressObj = { PackageName: '', TotalCount: '', CompletedPkgCount: '', EstimatedTimeString: '' };

        var processProgressFunc = function () {

            vm.stopChecking = false;

            vm.viewModelHelper.apiGet('api/runprocess/processprogressinfo', null,
                function (result) {
                    vm.processProgressObj = result.data;

                    vm.prProgLabel = true;

                    if (vm.processProgressObj.CompletedPkgCount < vm.processProgressObj.TotalCount) {
                        vm.spinner = true;

                        vm.processStatus = vm.processProgressObj.PackageName + ' Processing is ongoing: ' +
                            vm.processProgressObj.CompletedPkgCount + ' of ' + vm.processProgressObj.TotalCount + ' completed. ' +
                            ' Estimated Time: ' + vm.processProgressObj.EstimatedTimeString;

                        vm.processHasStarted = true;
                    }
                    else {
                        vm.spinner = false;

                        if (vm.processHasStarted === true && vm.processProgressObj.TotalCount === vm.processProgressObj.CompletedPkgCount) {

                            vm.processStatus = "Processing ends. Check log for detail.";
                            vm.stopChecking = true;
                        }
                    }
        ...
        var processProgressIndicatorForCancel = function () {
            vm.prProgLabel = false;
            vm.spinner = false;
            vm.processStatus = "";
        };
