...
(function () {
    ...
    function TSEditController($scope, $window, $state, $stateParams, viewModelHelper, validator, yearsService) {
        ...
        vm.module = 'MPR Core';
        vm.view = 'teamstructure-edit-view';
        ...
        var initialize = function () {
            if (vm.init === false) {
                ...
                if ($stateParams.Team_StructureId !== 0) {
                    ...
                    vm.viewModelHelper.apiGet('api/teamstructure/getteamstructure/' + $stateParams.Team_StructureId, null,
                        function (result) {
        ...
        vm.save = function () {
            ...
            if (vm.viewModelHelper.modelIsValid) {
                ...
                vm.viewModelHelper.apiPost('api/teamstructure/updatedetailstop1', vm.ts,
                    function (result) {
        ...
        vm.delete = function () {
            ...
            if (deleteFlag) {
                vm.viewModelHelper.apiPost('api/teamstructure/deleteteamstructure', vm.ts.Team_StructureId,
                    function (result) {
        ...
        vm.fillTheControlBoxes = function (teamcode, year) {
            //if (vm.init === false) {

            if (vm.ts.Team_Code === '' || vm.ts.Accountofficer_Code === '' || vm.ts.AccountofficerName === '' || vm.ts.staff_id === '' || vm.ts.Year === 0) {
                alert('AccountOfficer Name, AccountOfficer Code, StaffId, Year, Period and TeamCode cannot be null');
            }
            else {
                var typedTeamCode = vm.ts.Team_Code;
                var typedAcctOffCode = vm.ts.Accountofficer_Code;
                var typedAcctOffName = vm.ts.AccountofficerName;
                var typedStaffID = vm.ts.staff_id;
                var selectedYear = vm.ts.Year;
                var selectedPeriod = vm.ts.Period;

                vm.viewModelHelper.apiGet('api/teamstructure/getdetailstop1/' + teamcode + '/' + year, null,
                    function (result) {
                        ...
                        vm.ts.Accountofficer_Code = typedAcctOffCode;
                        vm.ts.AccountofficerName = typedAcctOffName;
                        vm.ts.staff_id = typedStaffID;
                        vm.ts.Year = selectedYear;
                        vm.ts.Period = selectedPeriod;
