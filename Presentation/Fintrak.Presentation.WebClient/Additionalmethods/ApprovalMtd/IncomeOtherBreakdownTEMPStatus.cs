...
{
    public class IncomeOtherBreakdownTEMPStatus
    {
        ...
        //================ methods to call starts ==========================================
        public IEnumerable<IncomeOtherBreakdownTEMPModel> IncomeOtherBreakdownTEMP(string status)
        {
            ...
            using (var con = new System.Data.SqlClient.SqlConnection(connectionString))
            {
                ...
                con.Open();
                cmd.CommandText = "select top (@TAKETOP) * from Income_Income_OtherBreakdown_TEMP where ApprovalStatus=@STATUS";
        ...
        public IEnumerable<IncomeOtherBreakdownTEMPModel> IncomeOtherBreakdownTEMPusingparams(string status, string search)
        {
            List<IncomeOtherBreakdownTEMPModel> obuList = new List<IncomeOtherBreakdownTEMPModel>();
            ...
            using (var con = new System.Data.SqlClient.SqlConnection(connectionString))
            {
                ...
                con.Open();
                cmd.CommandText = "select top 500 * from Income_Income_OtherBreakdown_TEMP where ApprovalStatus=@STATUS and " +
        ...
        public void EditIncomeOtherBreakdownApproval(string selectedIds)
        {
            using (var con = new System.Data.SqlClient.SqlConnection(connectionString))
            {
                using (var cmd = new System.Data.SqlClient.SqlCommand("", con))
                {
                    ...
                    con.Open();
                    var sql = "update Income_Income_OtherBreakdown_temp set ApprovalStatus='APPROVED', ApprovalUser=@CURRENTUSER where Id IN(";
        ...
        public void EditIncomeOtherBreakdownDecline(string selectedIds)
        {
            using (var con = new System.Data.SqlClient.SqlConnection(connectionString))
            {
                using (var cmd = new System.Data.SqlClient.SqlCommand("", con))
                {
                    ...
                    con.Open();
                    var sql = "update Income_Income_OtherBreakdown_temp set ApprovalStatus='DECLINED', ApprovalUser=@CURRENTUSER where Id IN(";
                    cmd.Parameters.AddWithValue("@CURRENTUSER", currentuser);
                    for (int i = 0; i < values.Length; i++)
                    {
                        sql = $"{sql} @{i},";
