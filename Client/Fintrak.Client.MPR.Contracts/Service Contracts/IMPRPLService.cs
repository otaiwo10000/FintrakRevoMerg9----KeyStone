...
namespace Fintrak.Client.MPR.Contracts
{
    ...
    public interface IMPRPLService : IServiceContract
    {
       ...
       [OperationContract]
        ...
        #region CommFeeManual

        [OperationContract]
        [TransactionFlow(TransactionFlowOption.Allowed)]
        IncomeAdjustmentCommFeeSearchManual UpdateCommFeeManual(IncomeAdjustmentCommFeeSearchManual CommFee);

        [OperationContract]
        [TransactionFlow(TransactionFlowOption.Allowed)]
        void DeleteCommFeeManual(int Id);

        [OperationContract]
        [FaultContract(typeof(NotFoundException))]
        IncomeAdjustmentCommFeeSearchManual GetCommFeeManual(int Id);

        [OperationContract]
        [FaultContract(typeof(NotFoundException))]
        IncomeAdjustmentCommFeeSearchManual[] GetCommFeeManuals();

        [OperationContract]
        //MPRCommFee[] GetMPRCommFeesBy(string searchType, string searchValue);
        IncomeAdjustmentCommFeeSearchManual[] GetCommFeesByYearPeriod(int year, int period, string search);
