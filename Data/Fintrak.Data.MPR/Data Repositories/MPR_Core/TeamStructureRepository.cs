...
{
    [Export(typeof(ITeamStructureRepository))]
    [PartCreationPolicy(CreationPolicy.NonShared)]
    public class TeamStructureRepository : DataRepositoryBase<TeamStructure>, ITeamStructureRepository
    {
        protected override TeamStructure AddEntity(MPRContext entityContext, TeamStructure entity)
        {
            return entityContext.Set<TeamStructure>().Add(entity);
        }

        protected override TeamStructure UpdateEntity(MPRContext entityContext, TeamStructure entity)
        {
            return (from e in entityContext.Set<TeamStructure>()
                    where e.Team_StructureId == entity.Team_StructureId
                    select e).FirstOrDefault();
        }

        protected override TeamStructure GetEntity(MPRContext entityContext, int id)
        {
            var query = (from e in entityContext.Set<TeamStructure>()
        ...
        protected override IEnumerable<TeamStructure> GetEntities(MPRContext entityContext)
        {
            return (from e in entityContext.Set<TeamStructure>()
                    select e).Take(50);
        }

        public IEnumerable<TeamStructure> GetTeamstructureByParams(string SearchValue, string year)
        {
        ...
        public IEnumerable<TeamStructure> GetTeamstructureByParameters(string selectedDefinitionCode, string SearchValue, string year)
        {
            using (MPRContext entityContext = new MPRContext())
            {
                List<TeamStructure> query = new List<TeamStructure>();
        ...
        public IEnumerable<TeamStructure> GetTeamstructureByDefinitionCodeMonthly(string code)
        {
            using (MPRContext entityContext = new MPRContext())
            {
                //string maxyear = entityContext.TeamStructureSet.Max(x => x.Year);
        ...
        ////NOTE: The team definition codes are hard coded here because they are not mapped/referenced in the teanstructure table.
        public IEnumerable<TeamStructure> GetTeamstructureByParamsAndeSetUpMonthly(string code, string SearchValue)
        {
            using (MPRContext entityContext = new MPRContext())
            {
                //string maxyear = entityContext.TeamStructureSet.Max(x => x.Year);
        ...
        public IEnumerable<TeamStructure> GetTeamstructureBySetUpMonthly()
        {
            using (MPRContext entityContext = new MPRContext())
            {
                //string maxyear = entityContext.TeamStructureSet.Max(x => x.Year);
        ...
        public TeamStructure GetTeamStructureTop1(string branch, string defcode, string year)
        {
            using (MPRContext entityContext = new MPRContext())
            {
                ...
                TeamStructure query = null;

                switch (defcode)
                {
                    case "TEM":
                        query = entityContext.TeamStructureSet.Where(x => x.Year.Trim() == year && (x.Team_Code.Trim().ToLower() == branch || x.TeamName.Trim().ToLower() == branch)).FirstOrDefault();
                        break;

                    case "BRH":
                        query = entityContext.TeamStructureSet.Where(x => x.Year.Trim() == year && (x.Branch_Code.Trim().ToLower() == branch || x.BranchName.Trim().ToLower() == branch)).FirstOrDefault();
                        break;

                    case "ZON":
                        query = entityContext.TeamStructureSet.Where(x => x.Year.Trim() == year && (x.Zone_Code.Trim().ToLower() == branch || x.ZoneName.Trim().ToLower() == branch)).FirstOrDefault();
                        break;
                }
